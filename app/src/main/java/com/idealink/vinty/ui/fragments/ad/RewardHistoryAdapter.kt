...
class RewardHistoryAdapter : ListAdapter<RewardHistory, RecyclerView.ViewHolder>(RewardHistoryDiffCallback()) {
    ...
    inner class EvenViewHolder(private val binding: ItemAdHistoryBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: RewardHistory) {
            binding.tickets.text = if (item.amount != null) {
                "+${item.amount} ${item.rewardType ?: "Reward"}"
            } else {
    ...
    class RewardHistoryDiffCallback : DiffUtil.ItemCallback<RewardHistory>() {
        override fun areItemsTheSame(oldItem: RewardHistory, newItem: RewardHistory) = oldItem.id == newItem.id
        override fun areContentsTheSame(oldItem: RewardHistory, newItem: RewardHistory) = oldItem == newItem
    }
}
